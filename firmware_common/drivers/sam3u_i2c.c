...
void TWI0_IrqHandler(void)
{
  ...
  u32InterruptStatus = TWI0->TWI_IMR;
  u32InterruptStatus &= TWI0->TWI_SR;

  /*** NACK Received (Master only) ***/
  if(u32InterruptStatus & TWI_SR_NACK )
  {
    /* Error has occurred, abort the message */
    TWI_u32Flags |= _TWI_ERROR_NACK;
    TWI_Peripheral0.pBaseAddress->TWI_IDR = TWI_IDR_ENDTX;
    TWI_Peripheral0.pBaseAddress->TWI_PTCR = TWI_PTCR_TXTDIS;
    TWI_pfnStateMachine = TwiSM_Error;
  }

  /*** ENDTX (transmit has finished) ***/
  if(u32InterruptStatus & TWI_SR_ENDTX )
  {
    ...
    {
      TWI_Peripheral0.pBaseAddress->TWI_CR = TWI_CR_STOP;
    }
  ...
  if(u32InterruptStatus & TWI_SR_ENDRX )
  {
    /* Disable interrupt and PDC transfer */
    TWI_Peripheral0.pBaseAddress->TWI_IDR = TWI_IDR_ENDRX;
    TWI_Peripheral0.pBaseAddress->TWI_PTCR = TWI_PTCR_RXTDIS;
  ...
  if(TWI_u8MsgQueueCount != 0)
  {
    if(TWI_psMsgBufferCurrent->eDirection == TWI_WRITE)
    {
      ...
      else
      {
        ...
        u32Byte = TWI_MMR_DADR(TWI_psMsgBufferCurrent->u8Address);
        ...
        TWI_Peripheral0.pBaseAddress->TWI_IER = TWI_IER_ENDTX;
        TWI_Peripheral0.pBaseAddress->TWI_PTCR = TWI_PTCR_TXTEN;
    ...
    else if(TWI_psMsgBufferCurrent->eDirection == TWI_READ)
    {
      ...
      u32Byte = TWI_MMR_MREAD | TWI_MMR_DADR(TWI_psMsgBufferCurrent->u8Address);
      TWI_Peripheral0.pBaseAddress->TWI_MMR |= u32Byte;
      ...
      if(TWI_psMsgBufferCurrent->u32Size == 1)
      {
        ...
        TWI_Peripheral0.pBaseAddress->TWI_CR = (TWI_CR_START | TWI_CR_STOP);
        TWI_pfnStateMachine = TwiSM_ReceiveLastByte;
      }
      else
      {
        ...
        TWI_Peripheral0.pBaseAddress->TWI_IER = TWI_IER_ENDRX;
        TWI_Peripheral0.pBaseAddress->TWI_PTCR = TWI_PTCR_RXTEN;

        /* Trigger the peripheral to start */
        TWI_Peripheral0.pBaseAddress->TWI_CR = TWI_CR_START;
...
static void TwiSM_TxWaitComplete(void)
{
  ...
  if(TWI_Peripheral0.pBaseAddress->TWI_SR & TWI_SR_TXCOMP)
  {
...
{
  if( TWI_Peripheral0.pBaseAddress->TWI_SR & TWI_SR_RXRDY )
  {
    /* Read the final byte */
