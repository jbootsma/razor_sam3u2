...
SspPeripheralType* SspRequest(SspConfigurationType* psSspConfig_)
{
  ...
  PMC->PMC_PCER0 |= (1 << psRequestedSsp->u8PeripheralId);
  ...
  if(psRequestedSsp->eSspMode == SSP_SLAVE)
  {
    ...
    psRequestedSsp->pBaseAddress->US_PTCR = (US_PTCR_RXTEN | US_PTCR_TXTEN);
    psRequestedSsp->pBaseAddress->US_IER  = (US_IER_CTSIC | US_IER_ENDRX | US_IER_ENDTX);
  }
  ...
  if(psRequestedSsp->eSspMode == SSP_SLAVE_FLOW_CONTROL)
  {
    ...
    psRequestedSsp->pBaseAddress->US_IER = US_IER_CTSIC;
  }
...
void SspInitialize(void)
{
  ...
  SSP_Peripheral0.pBaseAddress     = USART0;
  SSP_Peripheral0.u8PeripheralId   = ID_USART0;
  ...
  SSP_Peripheral1.pBaseAddress     = USART1;
  SSP_Peripheral1.u8PeripheralId   = ID_USART1;
  ...
  SSP_Peripheral2.pBaseAddress     = USART2;
  SSP_Peripheral2.u8PeripheralId   = ID_USART2;
...
static void SspGenericHandler(void)
{
  ...
  if( (SSP_psCurrentISR->pBaseAddress->US_IMR & US_IMR_CTSIC) &&
      (u32Current_CSR & US_CSR_CTSIC) )
  {
    ...
    if( (SSP_psCurrentISR->pCsGpioAddress->PIO_PDSR & SSP_psCurrentISR->u32CsPin) == 0)
    {
      ...
      *SSP_pu32SspApplicationFlagsISR &= ~(_SSP_TX_COMPLETE | _SSP_RX_COMPLETE);
      ...
      {
        SSP_psCurrentISR->pBaseAddress->US_IER |= US_IER_RXRDY;
      }
    ...
    else
    {
      ...
      *SSP_pu32SspApplicationFlagsISR &= ~_SSP_CS_ASSERTED;
      ...
      if(SSP_psCurrentISR->u32PrivateFlags & _SSP_PERIPHERAL_TX)
      {
        ...
        SSP_psCurrentISR->pBaseAddress->US_IDR = US_IDR_ENDTX;
  ...
  if( (SSP_psCurrentISR->pBaseAddress->US_IMR & US_IMR_TXEMPTY) &&
      (u32Current_CSR & US_CSR_TXEMPTY) )
  {
    ...
    else
    {
      ...
      SSP_psCurrentISR->pBaseAddress->US_IDR = US_IDR_TXEMPTY;
  ...
  if( (SSP_psCurrentISR->pBaseAddress->US_IMR & US_IMR_RXRDY) &&
      (SSP_psCurrentISR->pBaseAddress->US_CSR & US_CSR_RXRDY) )
  {
  ...
  if( (SSP_psCurrentISR->pBaseAddress->US_IMR & US_IMR_ENDRX) &&
      (u32Current_CSR & US_CSR_ENDRX) )
  {
        ...
        (SSP_psCurrentISR->eSspMode == SSP_MASTER_MANUAL_CS) )
    {
      ...
      SSP_psCurrentISR->pBaseAddress->US_PTCR = US_PTCR_RXTDIS | US_PTCR_TXTDIS;
      SSP_psCurrentISR->pBaseAddress->US_IDR  = US_IDR_ENDRX;
    }
  ...
  if( (SSP_psCurrentISR->pBaseAddress->US_IMR & US_IMR_ENDTX) &&
      (u32Current_CSR & US_CSR_ENDTX) )
  {
    ...
    if( (SSP_psCurrentISR->eSspMode == SSP_MASTER_AUTO_CS) ||
        (SSP_psCurrentISR->eSspMode == SSP_MASTER_MANUAL_CS) )
    {
      SSP_psCurrentISR->pBaseAddress->US_PTCR = US_PTCR_TXTDIS;
      SSP_psCurrentISR->pBaseAddress->US_IDR  = US_IDR_ENDTX;
    }

    /* Allow the peripheral to finish clocking out the Tx byte */
    u32Timeout = 0;
    while ( !(SSP_psCurrentISR->pBaseAddress->US_CSR & US_CSR_TXEMPTY) &&
    ...
    )
  {
   ...
   (SSP_MASTER_MANUAL_CS devices should already have asserted CS in the user's task) */
    ...
    if(SSP_psCurrentSsp->u16RxBytes !=0)
    {
      ...
      SSP_psCurrentSsp->pBaseAddress->US_IER = US_IER_ENDRX;

      /* Enable the receiver and transmitter to start the transfer */
      SSP_psCurrentSsp->pBaseAddress->US_PTCR = US_PTCR_RXTEN | US_PTCR_TXTEN;
    ...
    else
    {
      ...
      if(SSP_psCurrentSsp->eSspMode == SSP_SLAVE_FLOW_CONTROL)
      {
        ...
        SSP_psCurrentSsp->pBaseAddress->US_IDR = US_IDR_RXRDY;
        ...
        SSP_psCurrentSsp->pBaseAddress->US_CR = (US_CR_RSTTX);
        SSP_psCurrentSsp->pBaseAddress->US_CR = (US_CR_TXEN);
        SSP_psCurrentSsp->pBaseAddress->US_THR = (u8)u32Byte;
        SSP_psCurrentSsp->pBaseAddress->US_IER = US_IER_TXEMPTY;
      ...
      else
      {
        ...
        SSP_psCurrentSsp->pBaseAddress->US_IER = US_IER_ENDTX;

        /* Enable the transmitter to start the transfer */
        SSP_psCurrentSsp->pBaseAddress->US_PTCR = US_PTCR_TXTEN;
      }
